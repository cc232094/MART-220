...
function preload() {
    for (let i = 0; i < 10; i++) {
        ...
        characterFrames.push(loadImage(path, 
...
function draw() {
    ...
    if (characterFrames.length > 0) {
        image(characterFrames[animationIndex], characterX, characterY, characterWidth, characterHeight);
    }
    ...
    if (keyIsPressed) {
        if (key == 'a') sauceX -= 5;
        else if (key == 'd') sauceX += 5;
        else if (key == 'w') sauceY -= 5;
        else if (key == 's') sauceY += 5;
    }
